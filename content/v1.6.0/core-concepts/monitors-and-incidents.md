...
[expected response](/v1.6.0/core-concepts/assertions/) that decides healthy from broken. Uptimer
checks it on that interval from every [location](/v1.6.0/core-concepts/locations/) you assign —
the server, or a [remote worker](/v1.6.0/core-concepts/remote-workers/) — and each check is one
...
> produces a **monitoring subject**. [API v2](/v1.6.0/reference/rest-api/) and the Python SDK
...
the exact request/response fields, see the [REST API reference](/v1.6.0/reference/rest-api/).
...
| **No Data** | Not enough locations have reported to decide. Usually because **no [location](/v1.6.0/core-concepts/locations/) is assigned**, or because the workers serving them have gone quiet. |
...
[When an alert actually fires](/v1.6.0/alerting/slack-alerts/#when-an-alert-actually-fires).
...
**New in 1.5.0: the [REST API](/v1.6.0/reference/rest-api/) can set it too.** API v2 carries the
...
> [`server.sqids_salt`](/v1.6.0/operating/configuration/) — change that value and existing
