...
environment override — see [Configuration](/v1.4.0/operating/configuration/).
...
- **PostgreSQL** uses **versioned migrations** (schema changes + data backfills). By default they
  ...
  [`uptimer migrate`](/v1.4.0/operating/production/) job and set `boot_migrate: false` on the app
  containers — the schema is then brought to head atomically, once, instead of by each booting
  replica.

Background: [Choosing a database](/v1.4.0/operating/configuration/#choosing-a-database).
