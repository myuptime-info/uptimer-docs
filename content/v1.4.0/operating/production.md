...
Use PostgreSQL with one database — `uptimer_server`. Bring the schema to head with a one-shot job
before the app starts, and stop replicas migrating on boot:
...
[`examples/1.4.0/remote-workers`](https://github.com/myuptime-info/uptimer-docs/tree/main/examples/1.4.0/remote-workers)):
...
Only `uptimer_server` has to exist — since 1.4.0 the worker has no database of its own; it holds
its rules in memory and gets them from `grpc`. Its `/data` volume is still required, because that
is where its identity lives. `web` publishes only on loopback; terminate TLS and add real auth at
your reverse proxy.
...
[Remote workers](/v1.4.0/core-concepts/remote-workers/).
