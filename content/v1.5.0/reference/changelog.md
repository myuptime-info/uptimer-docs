...
title: "Changelog"
weight: 40
lede: "What changed for users in 1.5.0, and since 1.1."
description: "User-facing changes from 1.1 to 1.5.0."
---
...
- **A monitor's status is now decided across all of its locations**, not by the newest check to
  ...
  *At least one location*. A location that never reports counts as unknown and stays inside the
  ...
  answered. See [Monitors & incidents](/v1.5.0/core-concepts/monitors-and-incidents/#how-many-locations-must-fail).
...
- Signing out now also ends the session at your OIDC provider, so the next login no longer signs
  ...
  [Authentication](/v1.5.0/operating/authentication/#logging-out-of-the-provider).
...
- **PostgreSQL migrations are now versioned** (with data backfills) and upgrade-safe; SQLite
  ...
  [Choosing a database](/v1.5.0/operating/configuration/#choosing-a-database).
...
- **Assign regions when creating or updating a rule over the API** — the rule payload takes a
  `regions` field (region names), so a headless/[SDK](/v1.5.0/reference/python-sdk/) workflow can
  create a rule that actually runs instead of sitting at "No Data".
- New **`grpc_request_duration_seconds`** metric on [`/metrics`](/v1.5.0/reference/metrics/).
